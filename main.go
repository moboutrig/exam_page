...
// 		// Start the server
// 		fmt.Println("Server started at http://localhost:8633")
// 		err := http.ListenAndServe(":8633", nil)
// 		if err != nil {
...
func handleCompile(w http.ResponseWriter, r *http.Request) {
	...
	// Temporary directory to store the code files (use a path accessible in Termux)
	tmpDir := "/data/data/com.termux/files/home/compiler_task"
	err := os.MkdirAll(tmpDir+"/piscine", 0755)
	if err != nil {
		http.Error(w, "Failed to create directories", http.StatusInternalServerError)
		return
